...
from basic_info import origin, fdir_root, rltm_year, basic_order, level_required, var_name_list
from order_handler import get_all_valid_rltm_date, get_lead_str, get_var_list
...
if __name__ == "__main__":
    ...
    for ensemble_type in ["cf", "pf"]:
        ...
        rltm_date_list = list(get_all_valid_rltm_date(origin, rltm_year))
        for rltm_date in rltm_date_list:
            order["date"] = rltm_date.strftime("%Y-%m-%d")
                
            var_name_list = get_var_list(origin)
            for var_name in var_name_list:
                fname = f"{origin}.global.1P5.rltm.sd{rltm_date.strftime('%Y%m%d')}.{var_name}.{ensemble_type}.nc"
